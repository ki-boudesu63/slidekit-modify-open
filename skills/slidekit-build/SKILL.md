...
slide_plan.json の各スライドに `notes` フィールドを追加する。

**ノート生成の原則:**
1. スライドの内容を口語体で説明する文章
2. 1スライドあたり100〜200文字程度
3. 箇条書きではなく、実際に話す文章として記述
4. 専門用語の読み方や補足説明を含める
5. title / conclusion スライドにも挨拶文を入れる
...
```json
{
  ...
  "notes": "まず研究の背景からご説明します。近年、高齢化の進展に伴い、救急外来を受診する患者数は年々増加しています。一方で、初期トリアージの精度には施設間・担当者間でばらつきがあり、これが問題となっています。"
}
```

ノートの生成はデフォルトで行う。ユーザーが「ノートは不要」と言った場合はスキップする。
...
> 3. **発表ノートを編集する** — `notes_editor.html` をブラウザで開いて JSON を読み込み、各スライドの発表原稿を確認・編集してください
...
**発表者モード（プレゼン練習）:**
- index.html を開いた状態で `P` キーを押す → 発表者画面が別ウィンドウで開く
- 発表者画面にはスピーカーノート・次スライドプレビュー・経過時間が表示される
- どちらの画面からもスライド操作が可能（自動同期）
...
```json
{
  ...
  "slides": [
    ...
    { "type": "figure-focus", "heading": "結果", "image": "images/fig02.jpeg", "image_caption": "Figure 2", "body": "所見...", "notes": "こちらが主要な結果です。..." },
    { "type": "data-table", "heading": "データ", "body": "...", "table_html": "<table>...</table>", "table_caption": "Table 1", "notes": "データをご覧ください。..." },
    { "type": "section-break", "heading": "考察", "notes": "" },
...
| type | 説明 | 必須 | オプション |
|------|------|------|----------|
| `title` | タイトル（meta から自動生成） | — | `notes` |
| `conclusion` | 謝辞（meta から自動生成） | — | `notes` |
| `section-break` | セクション区切り（スライドのみ、ポスターでは無視） | `heading` | `notes` |
| `text-only` | テキストのみ | `heading`, `body` | `notes` |
| `two-column` | テキスト＋画像 | `heading`, `body`, `image`, `image_caption` | `notes` |
| `figure-focus` | 図メイン | `heading`, `image`, `image_caption` | `body`, `notes` |
| `data-table` | テーブル | `heading`, `table_html`, `table_caption` | `body`, `notes` |
