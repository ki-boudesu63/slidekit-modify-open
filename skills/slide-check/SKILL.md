...
ユーザーが「デザインを変えたい」「テーマを変えたい」「いい感じにして」と依頼した場合、`design/awesome-design-md/` にある有名企業のデザインシステムを参照してカラーを書き換える。
...
| ブランド | 特徴 | 向いている場面 |
|---------|------|------------|
| **stripe** | 白背景＋ネイビー＋パープルアクセント | 学会発表全般（おすすめ） |
| **notion** | 暖かいニュートラル、ミニマル | シンプル志向 |
| **linear.app** | ダークテーマ、インディゴアクセント | インパクト重視 |
| **vercel** | モノクロ＋ミニマル | テクノロジー系 |

### 変更手順

1. ユーザーにブランド名または方向性を聞く
2. `design/awesome-design-md/design-md/{ブランド}/DESIGN.md` を読む
3. 全スライドの `<style>` 内カスタムカラーを一括変更:

```bash
cd "<スライドフォルダ>"
sed -i 's/#2D1B69/#新色/g; s/#7C3AED/#新アクセント/g; s/#F3F0FF/#新ウォーム/g' 0*.html
```
