...
| **--export-md** | PDF 内容を Markdown + 両JSON に書き出し、`/slidekit-create` に渡す高品質モード |
...
| **slide_editor.html** | ビジュアルスライドエディタ。要素の移動・リサイズ・テキスト編集・図形挿入をGUIで実行 |
...
# ── 中間 JSON でレビュー・編集 ──
python -m builder paper.pdf --export-plan        # slide_plan.json + slide_content.json を出力
# → slide_reviewer.html で構成・素材を確認・編集してから生成可能

# ── 高品質モード（slidekit-create 連携） ──
python -m builder paper.pdf --export-md          # Markdown + 両JSON + 画像を書き出し
# → /slidekit-create に slide_content.json（推奨）または content.md を渡して
#   43 パターン × 5 スタイル × 5 テーマでスライド生成
...
flowchart TD
    ...
    PDF -->|"--export-md / --export-plan"| JSON["slide_plan.json<br/>slide_content.json<br/>content.md + images/"]
    JSON -->|"slide_reviewer.html"| REVIEW["レビュー・編集<br/>（構成モード / 素材モード）"]
    REVIEW -->|"編集済み JSON"| AUTO
    JSON -->|"slide_content.json"| CREATE["対話的生成<br/>（43パターン）"]
    PDF -->|"/slidekit-build"| PLAN["Claude 構成設計<br/>→ slide_plan.json<br/>+ slide_content.json"]
    PLAN -->|"レビュー後"| AUTO
...
Claude:
  1. PDF を読み取り
  2. 日本語 or 英語を確認
  3. slide_plan.json + slide_content.json を設計
  4. (任意) slide_reviewer.html で確認・編集
  5. python -m builder slide_plan.json を実行
  6. /slide-check で確認・修正
```

### 方法 3: 高品質モード（/slidekit-create 連携）

```bash
# 1. PDF → Markdown + JSON + 画像
python -m builder paper.pdf --export-md
# → slide_content.json, slide_plan.json, content.md, images/ が出力

# 2. (任意) slide_reviewer.html で素材を確認・編集
#    → ブラウザで開き、JSON を読み込んで内容・画像を調整

# 3. /slidekit-create で slide_content.json を参考ファイルとして指定
#    → 43 パターンからレイアウト選択
...
## ビジュアルスライドエディタ（slide_editor.html）

生成済みスライドをブラウザ上でGUI編集できるツール。`slide_editor.html` を開いてフォルダを選択するだけで使えます。

### 基本操作

| 操作 | 方法 |
|------|------|
| スライド読み込み | 「フォルダ」ボタンでスライドフォルダを選択 |
| スライド切り替え | 左パネルのサムネイルをクリック |
| 要素選択 | クリック |
| 親コンテナ選択 | Alt+クリック（繰り返しで上の階層へ） |
| 要素移動 | ドラッグ or 矢印キー（Shift+矢印で10px） |
| テキスト編集 | ダブルクリック |
| リサイズ | 選択ボックスのハンドルをドラッグ |
| 複数選択 | Shift+クリック or 範囲ドラッグ |
| 削除 | Delete キー |
| コピー/ペースト | Ctrl+C / Ctrl+V |
| 元に戻す/やり直し | Ctrl+Z / Ctrl+Y |
| 保存 | Ctrl+S（上書き）/ Ctrl+Shift+S（別名） |
...
選択した要素の位置・サイズ・フォント・色・透明度・角丸・枠線を数値で調整可能。整列ボタンでスライド内の位置揃えもできます。

### 特徴

- **フロー非破壊**: 要素の移動は `transform: translate()` で行うため、他の要素のレイアウトに影響しない
- **フォルダ読み込み対応**: 画像ファイルを含むスライドフォルダを丸ごと読み込み、相対パスを自動解決
- **File System Access API**: 対応ブラウザ（Chrome等）では直接上書き保存が可能
...
├── slide_editor.html                ← 【追加】ビジュアルスライドエディタ
...
│   ├── plan_exporter.py             #   【追加】JSON 書き出し（slide_plan / slide_content）
